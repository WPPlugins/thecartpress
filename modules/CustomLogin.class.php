...
class TCP_CustomLogin {
	...
	public static function tcp_activate_plugin() {
		...
		foreach ( $page_definitions as $page_key => $page ) {
	...
	public function redirect_to_custom_lostpassword() {
		if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
			...
			wp_redirect( self::get_page_url( 'tcp-member-password-lost' ) );
			exit;
		}
	}
	...
	public function redirect_to_custom_retrievepassword() {
		...
		if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
			...
			wp_redirect( self::get_page_url( 'tcp-member-password-reset' ) );
			exit;
		}
	}
	...
	private function redirect_logged_in_user( $redirect_to = null ) {
		...
		if ( user_can( $user, 'manage_options' ) ) {
			...
			wp_redirect( self::get_page_url( 'tcp_my_account' ) );
		}
	}
